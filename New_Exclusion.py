...
Results_ALL        = np.load('Numpys/Results_ALL_'+WHAT+'.npy').item()['Dics']         # Results for all points
Results_ATLAS02    = np.load('Numpys/Results_ATLAS02_' + WHAT + '.npy').item()['Dics'] # Results for ATLAS02 only
Masses             = np.load('Numpys/Masses_'+WHAT +'.npy').item()                     # ALL points exluded by ATLAS
...
for all, atlas,  glu in zip( Results_ALL , Results_ATLAS02, Gluino_Atlas_Off):
    all_r = all['Best_r']
    atlas_r =  atlas['Best_r']
...
# Extracting lightest squarks
Squark_Atlas_Off = Masses['Light_S']
Squark_Old          = Old_Excluded['Light_S']
Squark_All = []
Squark_ATLAS02eff = []
for all, atlas,  sq in zip( Results_ALL , Results_ATLAS02, Squark_Atlas_Off):
    all_r = all['Best_r']
    atlas_r =  atlas['Best_r']
    if all_r > r_limit_exclusion :
