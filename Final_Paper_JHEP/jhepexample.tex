...
\abstract{After the Run 1 at the Large Hadron Collider (LHC), the ATLAS collaboration analysed the constraints on a 19-parameters realisation of the phenomenological Minimal Supersymmetric Standard Model (pMSSM) using the results of the many searches for new physics. A large portion of the parameter space excluded by the ATLAS collaboration by means of full analyses recast, can be as well constrained by using  simplified model spectra results. They represent the standard method employed by the LHC Collaborations to interpret the results of the searches for Beyond the Standard Model (BSM) physics. A new class of simplified model stemming from gluino-squark associated production, producing a 3 jets + missing energy signature in the LHC detectors, has the potential to help constraining the uncovered part of the pMSSM parameter space. This work aims at demonstrating by producing results in the form of efficiency maps (EM) for the gluino-squark simplified model for existing LHC searches, it is possible to extend the pMSSM-19 coverage obtained in \cite{Ambrogi:2017lov} up to +19$\%$ and +8$\%$ depending on the Bino or Wino-like nature of the LSP. }
